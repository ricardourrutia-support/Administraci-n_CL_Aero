...
def generate_excel(df_grid: pd.DataFrame, date_start: date, date_end: date) -> io.BytesIO:
    ...
    subtype_order = {"Diurno": 0, "Nocturno": 1}
 
    people = (
        df_grid[["Nombre", "Rol"]]
        .drop_duplicates()
        .assign(
            order=lambda x: x["Rol"].map(role_order).fillna(9),
            subtype=lambda x: x["Nombre"].map(shift_class).fillna(""),
            subtype_order=lambda x: x["subtype"].map(subtype_order).fillna(0),
        )
        .sort_values(["order", "subtype_order", "Nombre"])
    )
    ...
    row = ROW_DATA_START
    current_group_key = None  # tracks "Rol" or "Rol-Subtype"
    group_title_rows = []
 
    fmt_subgroup = wb.add_format({
        "bold": True, "border": 1, "bg_color": "#F5F5F5",
        "align": "left", "indent": 2, "font_size": 9, "italic": True
    })
 
    for _, person in people.iterrows():
        nombre = person["Nombre"]
        rol = person["Rol"]
        subtype = person.get("subtype", "")
 
        # Determinar clave de grupo
        if rol in ("Agente", "Anfitrión") and subtype:
            group_key = f"{rol}-{subtype}"
        else:
            group_key = rol
 
        # Título de grupo principal (al cambiar de rol)
        if rol != current_group_key and rol not in (
            current_group_key.split("-")[0] if current_group_key and "-" in str(current_group_key) else ""
        ):
            label = {
        ...
        # Sub-título Diurno/Nocturno (al cambiar de sub-grupo)
        if group_key != current_group_key and rol in ("Agente", "Anfitrión") and subtype:
            sub_label = f"    ☀️ {subtype}s" if subtype == "Diurno" else f"    🌙 {subtype}s"
            ws.write(row, 0, sub_label, fmt_subgroup)
            ws.write(row, 1, "", fmt_subgroup)
            for c in range(2, total_cols):
                ws.write(row, c, "", fmt_subgroup)
            group_title_rows.append(row)
            row += 1
