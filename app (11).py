...
def generate_excel(df_grid: pd.DataFrame, date_start: date, date_end: date) -> io.BytesIO:
    ...
    for _, person in people.iterrows():
        ...
        for d in dates:
            ...
            if not is_working:
                ...
                for h in range(24):
                    cell_col = c_start + 2 + h
                    ws.write(row, cell_col, "", fmt["libre"])
                    # Dropdown HE para días libres (permite marcar hora extra)
    ...
    for d in dates:
        c_start = date_col_map[d]
        # La columna "Lugar" de este bloque está en c_start + 1
        lugar_col_letter = xlsxwriter.utility.xl_col_to_name(c_start + 1)
        ...
        for h in range(24):
            ...
            data_range_lugar = f"{lugar_col_letter}{r_start + 1}:{lugar_col_letter}{r_end + 1}"
            ...
            for counter_row, rol_name in [
                ...
                # Sumar T1 + T2 excluyendo filas con Lugar = "Ausente"
                formula = (
                    f'=COUNTIFS({data_range_rol},"{rol_name}",{data_range_task},"1",{data_range_lugar},"<>Ausente")'
                    f'+COUNTIFS({data_range_rol},"{rol_name}",{data_range_task},"2",{data_range_lugar},"<>Ausente")'
                )
    ...
    for d in dates:
        c_start = date_col_map[d]
        hours_range_start = c_start + 2
        hours_range_end = c_start + 25
