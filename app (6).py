...
def generate_excel(df_grid: pd.DataFrame, date_start: date, date_end: date) -> io.BytesIO:
    ...
    # ── Construir columnas por fecha ──
    col = 2
    date_col_map = {}  # fecha -> col_start (donde empieza el bloque)
    HHEE_ROWS = 5  # filas de HHEE al final
    ...
    for d in dates:
        ...
        for r in range(ROW_DATA_START - 2, ROW_DATA_START + len(people) + 50):
            ws.write(r, col, "", fmt["sep"])
        col += 1

        # Merge título fecha
        ws.merge_range(ROW_HEADER, col, ROW_HEADER, col + 1, d_label, fmt["date_bar"])
        ...
        for h in range(24):
            ...
            # Merge del header de fecha también sobre las horas
            if h == 0:
                ws.merge_range(ROW_HEADER, col + 2, ROW_HEADER, col + 25, "", fmt["date_bar"])
    ...
    for _, person in people.iterrows():
        ...
        # Título de grupo
        if rol != current_role:
            ws.merge_range(row, 0, row, col - 1, f"▸ {rol.upper()}ES" if rol != "Supervisor" else f"▸ {rol.upper()}ES", fmt["group_title"])
            # Corregir plural
            label = {
            ...
            ws.merge_range(row, 0, row, col - 1, label, fmt["group_title"])
            row += 1
            current_role = rol
    ...
    ws.merge_range(row, 0, row, col - 1, "▸ HORAS EXTRA / COBERTURAS MANUALES", fmt["group_title"])
    row += 1
...
if uploaded_file:
    ...
    if detected:
        st.sidebar.markdown("---")
        st.sidebar.header("3. Hojas detectadas")

        # Obtener mes del rango seleccionado para filtrar hojas
        ...
        for sn, role in detected:
            # Pre-seleccionar las hojas del mes target
            is_target = target_month.lower() in sn.lower() if target_month else True
