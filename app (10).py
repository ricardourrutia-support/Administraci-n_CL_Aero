...
def generate_excel(df_grid: pd.DataFrame, date_start: date, date_end: date) -> io.BytesIO:
    ...
    # Dotación activa = T1 + T2 (T2 es cobertura en otro lugar, no se pierde recurso)
    for d in dates:
        c_start = date_col_map[d]
        for h in range(24):
            h_col = c_start + 2 + h
            ...
            for counter_row, rol_name in [
                ...
                # Sumar T1 + T2 como dotación activa
                formula = (
                    f'=COUNTIFS({data_range_rol},"{rol_name}",{data_range_task},"1")'
                    f'+COUNTIFS({data_range_rol},"{rol_name}",{data_range_task},"2")'
                )
